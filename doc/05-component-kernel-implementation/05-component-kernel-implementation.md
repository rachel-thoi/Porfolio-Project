...
For the kernel implementation of my MusicPlaylist component, I will represent
the playlist using a `Sequence<String>` from the OSU components library, stored
in a field named `rep`. A sequence is a good fit because it maintains a specific
order of elements and already supports adding and removing entries at arbitrary
positions. This makes the kernel operations `addSong` (add to the end of the
playlist) and `removeSong` (remove the last song) straightforward and efficient
 to implement.
By convention, `rep` is never null, and every entry in `rep` is a non-null,
non-empty song title. The correspondence is that the abstract MusicPlaylist
value is exactly the sequence of songs stored in `rep`, in order from
index 0 (first song) to index `rep.length() - 1` (last song).
